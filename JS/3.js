...
;(function($, Drupal) {
    Drupal.behaviors.quicken_ccpa = {
        attach: function(context) {
            ...
            $('.ccpa-wrapper', context).once('ccpa-wrapper', function() {
                if (!!$.cookie("qkn_ccpa") && $.cookie("qkn_ccpa") == 1) {
                    $('.ccpa-wrapper', context).show();
                }
            });

            // Validate navigator do not Track
            if (navigator.doNotTrack === '1') {
                setCookie('qkn_dns', '1', 365);
            }
        }
    };
}(jQuery, Drupal));
